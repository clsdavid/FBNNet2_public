...
#' res
#' @export
generateFBMNetwork <- function(timeseries_data, method = c("kmeans", "edgeDetector", "scanStatistic"), maxK = 4, useParallel = FALSE, max_deep_temporal = 1, threshold_confidence = 1, 
    threshold_error = 0, threshold_support = 1e-05, maxFBNRules = 5, network_only = TRUE, verbose = FALSE) {
    ...
    if (verbose) {
        ...
        # This could be changed so that every detail is printed (not only errors and warnings). Do this by setting log_level argument to futile.logger::TRACE for full
        # info.
        
        # Now, we create an option of printing and storing results from the checks in somewhere other than the console. Below we are saying that if the log_appender
        # argument is not set to console (and set to a file name) then R will create a file to store the information and outputs from checks.
        
        # if(log_appender != 'console') { futile.logger::flog.appender(futile.logger::appender.file(log_appender)) }
    } else {
    ...
    futile.logger::flog.info(sprintf("Enter generateFBMNetwork zone: 
          ...
          network_only = %s,
          verbose = %s", 
        method, maxK, useParallel, max_deep_temporal, threshold_confidence, threshold_error, threshold_support, maxFBNRules, network_only, verbose))
    ...
    cube <- constructFBNCube(target_genes = genes, conditional_genes = genes, timeseriesCube = timeseries_data, maxK = maxK, temporal = max_deep_temporal, useParallel = useParallel)
    time2 <- as.numeric(Sys.time())
    print(paste("Total cost ", time2 - time1, " seconds to construct a FBN Cube.", sep = "", collapse = ""))
    time1 <- as.numeric(Sys.time())
    network <- mineFBNNetwork(fbnGeneCube = cube, threshold_confidence = threshold_confidence, threshold_error = threshold_error, threshold_support = threshold_support, 
        maxFBNRules = maxFBNRules, useParallel = useParallel)
    time2 <- as.numeric(Sys.time())
