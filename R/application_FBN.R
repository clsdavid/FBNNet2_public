...
#' network
#' @export
generateFBMNetwork <- function(timeseries_data, method = "kmeans", maxK = 4, useParallel = FALSE, parallel_on_group = FALSE, 
    max_deep_temporal = 7, threshold_confidence = 1, threshold_error = 0, threshold_support = 1e-05, maxFBNRules = 5, maxGenesForSingleCube = 20, 
    runtype = 0, network_only = TRUE) {
    ...
    futile.logger::flog.info(sprintf("Enter generateFBMNetwork zone: method=%s, maxK=%s, useParallel=%s, 
        ...
        method, maxK, useParallel, parallel_on_group, max_deep_temporal, threshold_confidence, threshold_error, threshold_support, 
        maxFBNRules, maxGenesForSingleCube))
    ...
    if (runtype == 0) {
        ...
        network <- mineFBNNetwork(cube, maxFBNRules = maxFBNRules, useParallel = useParallel)
        return(network)
    }
    
    if (runtype == 1) {
        futile.logger::flog.info(sprintf("Run generateFBMNetwork with cimbined clustering"))
        cube_cluster <- clusterdDiscreteData(raw_timeseries_data, timeseries_data, minElements = 2, maxElements = maxGenesForSingleCube)
        this_cube_cluster <- constructFBNCubeAndNetworkInClusters(cube_cluster, maxK = maxK, temporal = max_deep_temporal, 
            useParallel = useParallel, parallel_on_group = parallel_on_group, output_cube = FALSE)
    } else if (runtype == 2) {
        ...
        this_cube_cluster <- constructFBNCubeAndNetworkInSmallGroups(cube_cluster, maxK = maxK, temporal = max_deep_temporal, 
            useParallel = useParallel, parallel_on_group = parallel_on_group, output_cube = FALSE)
    } else {
    ...
    network <- mergeClusterNetworks(clusteredFBNCube = this_cube_cluster, threshold_error = threshold_error, maxFBNRules = maxFBNRules)
