...
convertToBooleanNetworkCollection <- function(network) {
  ...
  merges <- mergeInteraction(lapply(network$interactions, convertInteraction), lapply(network$interactions, convertInteraction), genes1, genes2, 
    totalGenes)
...
convertInteraction <- function(interaction) {
  ...
  if (mode(interaction) == "list") {
    ...
    res <- list(list(input = interaction$input, expression = interaction$expression, error = interaction$error, type = NA, probability = (1 - 
      as.numeric(interaction$error)), support = NA, timestep = 1))
  }
  return(res)
}
...
findBackwardRelatedNetworkByGenes <- function(networks, target_gene_list = c(), regulationType = NULL, maxDeep = 1, next_level_mix_type = FALSE) {
  ...
  for (target_gene in target_gene_list) {
    ...
    new_nextwork <- findAllBackwardRelatedGenes(networks = networks, target_gene = target_gene, regulationType = regulationType, target_type = target_type, 
      maxDeep = maxDeep, next_level_mix_type = next_level_mix_type)
...
#' @param maxDeep The maximum layous that need to be drilled down.
#' @param next_level_mix_type If TRUE, the next level will contain all type of connections.
findAllForwardRelatedGenes <- function(networks, target_gene, regulationType = NULL, target_type = NULL, main_target_gene = NULL, main_target_type = NULL, 
  maxDeep = 1, next_level_mix_type = next_level_mix_type) {
  ...
  if (maxDeep > 1 && length(prepare_network$interactions) > 0) {
    ...
    for (this_target_gene in expand_genes) {
      if (next_level_mix_type) {
        new_nextwork <- findAllForwardRelatedGenes(networks = networks, target_gene = this_target_gene, regulationType = 1, target_type = target_type, 
        ...
        new_nextwork <- findAllForwardRelatedGenes(networks = networks, target_gene = this_target_gene, regulationType = 0, target_type = target_type, 
          main_target_gene = main_target_gene, main_target_type = main_target_type, maxDeep = maxDeep, next_level_mix_type = next_level_mix_type)
        prepare_network <- filterNetworkConnections(mergeNetwork(prepare_network, new_nextwork))
      } else {
        new_nextwork <- findAllForwardRelatedGenes(networks = networks, target_gene = this_target_gene, regulationType = regulationType, 
          target_type = target_type, main_target_gene = main_target_gene, main_target_type = main_target_type, maxDeep = maxDeep, next_level_mix_type = next_level_mix_type)
        prepare_network <- filterNetworkConnections(mergeNetwork(prepare_network, new_nextwork))
      }
    }
  }
...
findForwardRelatedNetworkByGenes <- function(networks, target_gene_list = c(), regulationType = NULL, target_type = NULL, maxDeep = 1, next_level_mix_type = FALSE) {
  ...
  prepare_network <- NULL
  for (target_gene in target_gene_list) {
    new_nextwork <- findAllForwardRelatedGenes(networks = networks, target_gene = target_gene, regulationType = regulationType, target_type = target_type, 
      main_target_gene = target_gene, main_target_type = target_type, maxDeep = maxDeep, next_level_mix_type = next_level_mix_type)
    if (is.null(prepare_network)) {
      prepare_network <- new_nextwork
    } else {
      prepare_network <- mergeNetwork(prepare_network, new_nextwork)
