...
clusterTimeSeries <- function(timeSeriesData, 
                              method = c("kmeans", "hierarchical", "diana", "fanny", "nbclust"), 
                              modelParameters) {
  # require("cluster")
  # require("factoextra")
  # require("magrittr")
  ...
  switch(match.arg(method), kmeans = {
    ...
    library(factoextra)
    fviz_dend(thisCluster, k = numOfCluster, cex = 0.5, k_colors = c("#2E9FDF", "#00AFBB", "#E7B800", "#FC4E07"), color_labels_by_k = TRUE, rect = TRUE  # Add rectangle around groups
    )
    ...
    factoextra::fviz_cluster(thisCluster, ellipse.type = "norm", repel = TRUE, palette = "jco", ggtheme = ggplot2::theme_minimal(), legend = "right")
    ...
    thisCluster <- na.omit(clusterData) 
    thisCluster <- scale(thisCluster) 
    thisCluster <- NbClust(thisCluster, 
                           distance = distmethod, 
                           min.nc = min.nc,
                           max.nc = max.nc, 
                           method = nbmethod,
