                                      ...
                                      Rcpp::Nullable<Rcpp::List> targetCounts)
{
  ...
  for(int i=0; i < getAllTemporalStates.length(); i++){
     ...
     Rcpp::NumericMatrix computation_Matrix = temporalState["computation_Matrix"];
     Rcpp::NumericMatrix computation_Matrix_c = temporalState["computation_Matrix_c"];
     total_calculated_timepoints = n_timepoints-(total_samples * time_step);
     List result = getBasicMeasures(stateTCond,
                                    computation_Matrix,
