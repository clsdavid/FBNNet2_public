    ...
    bool findNegativeRegulate = false)
{
 //tree mining stop when 1 or 0
  ...
  if(!matchedexpression.isNull())
  {
   ...
   CharacterVector activator = CharacterVector::create(_["factor"]=pick_expT, _["Confidence"]=(String)pickT_value, _["ConfidenceCounter"]=(String)pickT_confidenceCounter,
                                                       _["all_confidence"]=(String)pickT_all_confidence, _["max_confidence"]=(String)pickT_max_confidence, _["support"]=(String)pickT_support,
                                                       _["causality_test"]=(String)pickT_causality_test, _["Noise"]=(String)pickT_noise, _["Identity"]=identity_T,
                                                       _["type"]=sign_P, _["timestep"]=(String)timestep_T,
                                                       _["isNegativeCorrelated"]=(String)isNegativeCorrelated_T, _["isPossitiveCorrelated"]=(String)isPossitiveCorrelated_T, _["bestFitP"]=(String)bestFitP);
   ...
   CharacterVector inhibitor = CharacterVector::create(_["factor"]=pick_expF, _["Confidence"]=(String)pickF_value, _["ConfidenceCounter"]=(String)pickF_ConfidenceCounter,
                                                       _["all_confidence"]=(String)pickF_all_confidence, _["max_confidence"]=(String)pickF_max_confidence, _["support"]=(String)pickF_support,
                                                       _["causality_test"]=(String)pickF_causality_test,_["Noise"]=(String)pickF_noise,_["Identity"]=identity_F,
                                                       _["type"]=sign_N, _["timestep"]=(String)timestep_F,
                                                       _["isNegativeCorrelated"]=(String)isNegativeCorrelated_F, _["isPossitiveCorrelated"]=(String)isPossitiveCorrelated_F, _["bestFitN"]=(String)bestFitN);
      ...
      bool findNegativeRegulate = false)
{
   ...
   for(int i=0;i<len;i++){
      ...
      if(pmaxK>1 && !(findPositiveRegulate && findNegativeRegulate))
      {
         ...
         if(nextGenes_T.length()>0 && is_essential_gene_P && (bestFit_P>0 || bestFit_N>0))
         {
            subresultT=mineNetworksDirect(targetGene,mainParameters,nextGenes_T,newMatchedGenesT,CharacterVector::create(expT),IntegerVector::create(pmaxK),temporal, new_targetCounts, findPositiveRegulate, findNegativeRegulate);
         }
         ...
         if(nextGenes_F.length()>0 && is_essential_gene_N && (bestFit_P>0 || bestFit_N>0))
         {
            subresultF=mineNetworksDirect(targetGene,mainParameters,nextGenes_F,newMatchedGenesF,CharacterVector::create(expF),IntegerVector::create(pmaxK),temporal, new_targetCounts, findPositiveRegulate, findNegativeRegulate);
         }
      }
      ...
      List inhibitors;
      if(subresultT.length()>0){
         activators = subresultT["activators"];
      }
      
      if(subresultF.length()>0){
         inhibitors = subresultF["inhibitors"];
      }
      ...
      if (bestFit_P == 0) {
         inhibitors.push_back(inhibitors);
      }
      int aindex = activators.length();
      if(aindex > 0) {
         List res_a = res[0];
         int raindex = res_a.length();
         res_a = resizel(res[0], raindex + aindex);
         res_a[Range(raindex, res_a.length() -1)] = activators;
         res[0] = res_a;
      }

      
      int iindex = inhibitors.length();
      if(iindex > 0) {
         List res_i = res[1];
         int riindex = res_i.length();
         res_i = resizel(res[1],riindex + iindex);
         res_i[Range(riindex, res_i.length() -1)] = inhibitors;
         res[1] = res_i;
      }
   }
   res.attr("names") = CharacterVector::create("activators", "inhibitors");;
   res = removeEmptyElement(res);
   return(res);
}
